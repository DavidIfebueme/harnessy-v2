...
describe("runDcrConnect", () => {
  it("names dynamically registered OAuth apps as Executor clients", () => {
    expect(dcrClientNameForIntegration("PostHog MCP")).toBe("Harnessy for PostHog MCP");
    expect(dcrClientNameForIntegration("   ")).toBe("Harnessy");
  });
  ...
  it("auto-registers (no picker) then starts: probe → register → start in order", async () => {
    ...
    expect(registerArgs!.clientName).toBe("Harnessy for Linear MCP");
