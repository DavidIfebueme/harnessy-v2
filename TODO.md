...
- Done: native `CommandRunner` service (effect-smol `ChildProcessSpawner` seam) executes runnable external bootstrap commands (git source refresh, `uv tool install`) as explicit argv arrays — never shell strings — behind `--apply-bootstrap --run-external`. Compound/piped v1 commands (`curl | sh`, corepack `&&`) stay manual. Tested deterministically with a recording fake spawner (opencode pattern); no real binaries, network, or timing in unit tests.
